...
echo "[iDevOps] Hadolint -- Dockerfile Linting"
echo "----------------------------------------"
...
if [ -z "$DOCKERFILES" ]; then
  echo "[iDevOps] No Dockerfiles found, skipping"
  exit 0
fi

# Install Hadolint if not present
if ! command -v hadolint &>/dev/null; then
  echo "[iDevOps] Installing Hadolint..."
...
for dockerfile in $DOCKERFILES; do
  echo "[iDevOps] Linting: $dockerfile"
...
echo ""
echo "[iDevOps] Hadolint issues: $TOTAL_ISSUES"

if [ "$EXIT_CODE" -ne 0 ]; then
  if [ "$FAIL_ON" = "critical" ] || [ "$FAIL_ON" = "high" ]; then
    echo "[iDevOps] FAIL: Dockerfile issues found (threshold: $FAIL_ON)"
    exit 1
  fi
fi
echo "[iDevOps] PASS: Hadolint passed (threshold: $FAIL_ON)"
